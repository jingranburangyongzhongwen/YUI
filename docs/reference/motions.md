...
- **Horizontal root lock** (`root_lock_xz`, default `false`): a `true` entry has its hips X/Z pinned to the origin instead of mean-recentered, and the floor path that came out is kept beside the clip. The mover replays that path by translating the OS window; hips +Z is not replayed (scale-as-depth reads as the model growing). Mean-recentering would leave relative XZ travel in the track and the pet would walk off the canvas. The flag rides in the clip-cache key with `root_lock_y`. Local custom clips use this.
