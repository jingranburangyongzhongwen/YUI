...
- **Horizontal root lock** (`root_lock_xz`, default `false`): a `true` entry has its hips X/Z pinned to the origin instead of mean-recentered, and the floor path that came out is kept beside the clip. The mover replays that path by translating the OS window; hips +Z is not replayed (scale-as-depth reads as the model growing). Mean-recentering would leave relative XZ travel in the track and the pet would walk off the canvas. The mover yields the window while perched, peeking, dragging, walking to a target, or a travel is current. A published oneshot while perched stands her up off the ledge first, then plays as it would on the floor, so an xz-locked clip can move the window; when the clip ends she sits back, or falls if that host is gone. A oneshot on a peek still plays in place. Idle returns, ambient, and reactive locomotion stay suppressed while held. The flag rides in the clip-cache key with `root_lock_y`. Local custom clips use this.
- **Local custom clips**: `.vrma` files in [`public/custom_motions/`](../../public/custom_motions/AGENTS.md) (gitignored). At load, each stem becomes a published oneshot (`root_lock_xz: true`) merged onto the catalog, so the agent can pick it via `generate_express` `motion_id` (`spin.vrma` → `spin`). They show in the Character tab (`custom` group) and are not a source in this catalog. Dropping a WHAM `.pkl` onto the character silhouette converts it through `pkl2vrma` into this folder, hot-merges the stem, and plays it. Dropping a video (`.mp4` / `.mov`) uploads it to the configured WHAM HTTP service (`endpoints.wham_base_url`), downloads the pkl, and runs the same convert-and-install at the video's frame rate so playback speed matches the source; an unset URL leaves the video drop unimported. A file whose stem collides with the shipped catalog is stored under a disambiguated id. A Finder or Explorer file-drag drops click-through for the gesture so the webview can take the drop; Windows also forwards a parent-HWND drop as `os_file_drop`. Release on the silhouette plays `happy` and a "Learning a dance…" chip, then loops `idle_lively` until the new stem starts; a failed convert plays `sheepish`. Sitting or peeking skips the floor poses and plays the new stem when it is ready.
...
| `stand_up` | oneshot | no | Standing up from a window seat, played before a perched stroll, a climb descent, or a published oneshot leaves the seat. `root_lock_y: true`: the pin is released and the window follows the clip's rise, scaled to the same seat distance, so the feet end on the edge. `broker_publish: false`. | Mixamo "Sit To Stand" | 2.03s |
