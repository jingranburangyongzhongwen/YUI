...
vi.mock("../../ambient/locomotion/wire", () => ({
  ...
  wireRootMover: mocks.wireRootMover,
  wireLeaveSeatThenPlay: mocks.wireLeaveSeatThenPlay,
}));
...
describe("wireLocomotion", () => {
  ...
  const setup = () => {
    ...
    const rootMover = {
      cancel: () => cancelOrder.push("rootMover.cancel"),
      dispose: () => teardowns.push("rootMover.dispose"),
    };
    const climber = {
    ...
    mocks.wireRootMover.mockImplementation(() => rootMover);
    mocks.wireLeaveSeatThenPlay.mockImplementation(() => () => teardowns.push("leave-seat"));
  ...
  it("registers the teardowns in composition order", () => {
    ...
    expect(s.registered).toHaveLength(11);
