...
import type { MotionRegistry, MotionRegistryEntry } from "../../contract";
...
export interface MotionPlayback extends VrmParticipant {
  ...
  upsert(id: string, entry: MotionRegistryEntry): void;
...
export function createMotionPlayback(deps: {
  ...
  async function startMotion(motion: ResolvedMotion): Promise<void> {
    ...
    if (!vrm || !mixer) return;
    try {
      let clip = await deps.clips.load(motion.vrma_path, motion.root_lock_y, motion.root_lock_xz);
  ...
  function upsert(id: string, entry: MotionRegistryEntry): void {
